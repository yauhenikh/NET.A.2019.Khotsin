...
namespace BookListServiceLibrary
{
    ...
    public class BinaryFileStorage : AbstractStorage
    {
        ...
        {
            base.AddBook(book);
            using (BinaryWriter writer = new BinaryWriter(File.Open(this._filePath, FileMode.OpenOrCreate)))
            {
        ...
        public override void RemoveBook(Book book)
        {
            ...
            using (BinaryWriter writer = new BinaryWriter(File.Open(this._filePath, FileMode.Truncate)))
            {
                writer.Seek(0, SeekOrigin.End);

                foreach (Book bookInList in this._Books)
                {
